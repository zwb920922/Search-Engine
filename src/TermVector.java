...
/**
 * An Indri DocVector-style interface for the Lucene termvector. There are three
 * main data structurs:
 * <p>
 * 
 * <pre>
...
public class TermVector {
  ...
  private Map<String, Integer> stemIndexMap; // used for finding the index of a
                                             // stem
  ...
  public TermVector(int docId, String fieldName) throws IOException {
    ...
    // Create and fill the positions array. Since we have position indexes, the
    // array's size needs
    ...
    stemIndexMap = new HashMap<>();
    for (int i = 0; i < stems.length; i++) {
      if (stems[i] != null)
        stemIndexMap.put(stems[i], i);
    }
  }

  /**
   * Get the index of stem in the stems vector, or -1 if the stems vector does
   * not contain the stem.
   * 
   * @param stem
   *          The stem to search for.
   * @return the index of the stem in the stems vector, or -1 if it does not
   *         occur.
   */
  public int indexOfStem(String stem) {
    
    Integer index = stemIndexMap.get(stem);
