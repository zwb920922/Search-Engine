...
public class QrySopWand extends QrySop implements WeightedQry {
    ...
    if (!this.docIteratorHasMatchCache()) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        if (q_i.docIteratorHasMatch(r) && q_i.docIteratorGetMatch() == docid)
          ret *= Math.pow(((QrySop) q_i).getScore(r), weight/weightSum);
        else
          ret *= Math.pow(((QrySop) q_i).getDefaultScore(r, docid), weight/weightSum);
      }
      return ret;
    }
  }
