...
/**
 * A simple utility for inspecting Lucene indexes. Run it to see a simple usage
 * message.
 */
public class InspectIndex {
  ...
  public static void main(String[] args) throws IOException {
    ...
    /*
     * Opening the index first simplifies the processing of the rest of the
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      if ("-index".equals(args[i])) {
        ...
        System.out.println(
            "\turl:\t" + "\tnumdocs=" + reader.getDocCount("url") + "\tsumTotalTF=" + reader.getSumTotalTermFreq("url")
                + "\tavglen=" + reader.getSumTotalTermFreq("url") / (float) reader.getDocCount("url"));
  ...
  /*
   * listPostings displays the first n postings for a term in a field in an
   * index (specified by reader). Set n to MAX_VALUE to display all postings.
   */
  static void listPostings(IndexReader reader, String termString, String field, Integer n) throws IOException {
     ...
     */
    DocsAndPositionsEnum postings = MultiFields.getTermPositionsEnum(reader, liveDocs, field, termBytes);
  ...
  static void listTermDictionary(IndexReader reader, String fieldName) throws IOException {
    ...
    /*
     * Grant says: MultiFields.getTerms(IndexReader, fieldName)
     */
    ...
    if ((terms == null) || (terms.size() == -1))
      ...
      /*
       * Iterate over the terms in this document. Information about a term's
       * occurrences (tf and positions) is accessed via the indexing API, which
       * returns inverted lists that describe (only) the current document.
       */
  ...
  /*
   * listTermVectors displays the term vectors for all of the fields in a
   * document in an index (specified by reader).
   */
  ...
  /*
   * listTermVectorField displays the term vector for a field in a document in
   * an index (specified by reader).
   */
  ...
  static void termVectorDisplay(Terms terms) throws IOException {
    ...
    if ((terms == null) || (terms.size() == -1))
       ...
       */
      while (ithTerm.next() != null) {
        System.out.format("      %10d %-20s %d ", ithTerm.ord(), ithTerm.term().utf8ToString(),
            ithTerm.totalTermFreq());
